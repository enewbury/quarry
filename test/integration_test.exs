  ...
  describe "preloads" do
    test "preloads data" do
      ...
      assert [%{id: ^id, author: %{id: ^author_id, user: %{id: ^user_id}}}] =
               Context.list_posts(preloads: preloads)
    end

    test "preloads has_many relationship" do
      post = insert(:post)
      %{id: comment_id} = insert(:comment, post: post)

      preloads = [:comments]

      assert [%{comments: [%{id: ^comment_id}]}] = Context.list_posts(preloads: preloads)
    end

    test "preloads nested has_many relationship" do
      user = insert(:user)
      author = insert(:author, user: user)
      post = insert(:post, author: author)
      insert(:comment, post: post, user: user)

      preloads = [:user, post: [author: [posts: :author]]]

      assert [%{user: _user, post: %{author: %{posts: [%{author: _author}]}}}] =
               Context.list_comments(preloads: preloads)
    end
  end
  ...
  describe "filter" do
    ...
    test "filter nested attribute" do
      ...
      assert [%{id: ^id, author: %{publisher: ^publisher}}] =
               Context.list_posts(filter: filter, preloads: [:author])
    end
    ...
    test "filter has_many attribute" do
      ...
      assert [%{id: ^post_id, comments: [%{body: "comment"}]}] =
               Context.list_posts(filter: filter, preloads: :comments)
    end
    ...
    test "limit has_many attribute" do
      ...
      assert [%{comments: [%{body: "comment2"}]}] =
               Context.list_posts(preloads: [comments: [limit: 1, offset: 1]])
    end
  end
  ...
  describe "sort" do
    ...
    test "can order multiple top and nested attribute" do
      ...
      assert [%{title: "A", author: %{publisher: "B"}}, %{title: "A"}, %{title: "B"}] =
               Context.list_posts(sort: [:title, author: :publisher], preloads: :author)
    end
    ...
    test "can sort desc" do
      ...
      assert [%{title: "C"}, %{title: "B"}, %{title: "A"}] =
               Context.list_posts(sort: %{direction: :desc, value: :title})
    end
  end
end
